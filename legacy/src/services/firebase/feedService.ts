...
/**
 * One-shot fetch of friends who are currently at a playground (status !== 'left').
 * Returns a map of playgroundId → active friend feed entries.
 * Used to surface "friend is here right now!" on CheckInScreen.
 */
export async function getFriendsAtPlaygrounds(
  userId: string
): Promise<Map<string, FeedEntry[]>> {
  ...
  snap.docs.forEach((doc) => {
    ...
    const existing = map.get(entry.playgroundId) ?? [];
    existing.push(entry);
    map.set(entry.playgroundId, existing);
  });
  return map;
}
